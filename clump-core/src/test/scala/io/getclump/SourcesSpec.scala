...
class SourcesSpec extends Spec {
  ...
  "creates a clump source" >> {
    ...
    "extra params" >> {
      ...
      "three" in {
        ...
        val source = Clump.source(fetch _)
        val clump = Clump.collect(source.get(1, "2", List("a"), 3),
                                  source.get(1, "2", List("a"), 4),
                                  source.get(2, "3", List("b"), 5))
        clumpResult(clump) mustEqual Some(
          List(List(1, "2", List("a"), 3), List(1, "2", List("a"), 4), List(2, "3", List("b"), 5)))
      }
      "four" in {
        ...
        val source = Clump.source(fetch _)
        val clump = Clump.collect(source.get(1, "2", List("a"), true, 3),
                                  source.get(1, "2", List("a"), true, 4),
                                  source.get(2, "3", List("b"), false, 5))
        clumpResult(clump) mustEqual Some(
          List(List(1, "2", List("a"), true, 3), List(1, "2", List("a"), true, 4), List(2, "3", List("b"), false, 5)))
      }
    }
  }
    ...
    "extra params" >> {
      ...
      "four" in {
        ...
        val clump = Clump.collect(source.get(1, "2", List("a"), true, 3),
                                  ...
                                  source.get(2, "3", List("b"), false, 5))
        clumpResult(clump) mustEqual Some(
          List((1, "2", List("a"), true, 3), (1, "2", List("a"), true, 4), (2, "3", List("b"), false, 5)))
      }
    }
  }
  ...
  "creates a clump source that allows marking individual entries as failed" >> {
    ...
    "extra params" >> {
      ...
      "two" in {
        def fetch(param1: Int, param2: Int, inputs: List[Int]) =
          Future.successful(inputs.map(x => x -> Try(1 / (x + param1 + param2))).toMap)
        val source = Clump.sourceTry(fetch _)
        clumpResult(source.get(1, 0, 0)) mustEqual Some(1)
        clumpResult(source.get(1, 1, -2)) must throwA[ArithmeticException]
      }
      "three" in {
        def fetch(param1: Int, param2: Int, param3: Int, inputs: List[Int]) =
          Future.successful(inputs.map(x => x -> Try(1 / (x + param1 + param2 + param3))).toMap)
        val source = Clump.sourceTry(fetch _)
        clumpResult(source.get(1, 0, 0, 0)) mustEqual Some(1)
        clumpResult(source.get(1, 1, 1, -3)) must throwA[ArithmeticException]
      }
      "four" in {
        def fetch(param1: Int, param2: Int, param3: Int, param4: Int, inputs: List[Int]) =
          Future.successful(inputs.map(x => x -> Try(1 / (x + param1 + param2 + param3 + param4))).toMap)
        val source = Clump.sourceTry(fetch _)
  ...
  "creates a clump source with zip as the key function" >> {
    ...
    "extra params" >> {
      ...
      "two" in {
        def fetch(param1: Int, param2: String, inputs: List[Int]) =
          Future.successful(inputs.map(_ + param1).map(_ + param2))
        val source = Clump.sourceZip(fetch _)
        clumpResult(source.get(1, "a", 2)) mustEqual Some("3a")
      }
      "three" in {
        def fetch(param1: Int, param2: String, param3: List[String], inputs: List[Int]) =
          Future.successful(inputs.map(_ + param1).map(_ + param2).map(_ + param3.fold("")(_ + _)))
        val source = Clump.sourceZip(fetch _)
        clumpResult(source.get(1, "a", List("b", "c"), 2)) mustEqual Some("3abc")
      }
      "four" in {
        def fetch(param1: Int, param2: String, param3: List[String], param4: Boolean, inputs: List[Int]) =
          Future.successful(inputs.map(_ + param1).map(_ + param2).map(_ + param3.fold("")(_ + _)).map(_ + s"-$param4"))
        val source = Clump.sourceZip(fetch _)
  ...
  "creates a clump source from a singly keyed fetch function" >> {
    ...
    "function with failure" in {
      def fetch(input: Int) =
        if (input % 2 == 0) Future.successful(input / 2) else Future.failed(new ArithmeticException)
    ...
    "extra params" >> {
      ...
      "three" in {
        def fetch(param1: String, param2: List[Int], param3: Int, input: Int) =
          Future.successful(input + param1 + param2.mkString + param3)
        val source = Clump.sourceSingle(fetch _)
        clumpResult(source.get("a", List(1, 2), 3, 4)) mustEqual Some("4a123")
      }
      "four" in {
        def fetch(param1: String, param2: List[Int], param3: Int, param4: List[String], input: Int) =
          Future.successful(input + param1 + param2.mkString + param3 + param4.mkString)
        val source = Clump.sourceSingle(fetch _)
  ...
  "creates a clump source from various input/ouput type fetch functions" in {
    def setToSet: Set[Int] => Future[Set[String]] = { inputs =>
      Future.successful(inputs.map(_.toString))
    }
    def listToList: List[Int] => Future[List[String]] = { inputs =>
      Future.successful(inputs.map(_.toString))
    }
    def iterableToIterable: Iterable[Int] => Future[Iterable[String]] = { inputs =>
      Future.successful(inputs.map(_.toString))
    }
    def setToList: Set[Int] => Future[List[String]] = { inputs =>
      Future.successful(inputs.map(_.toString).toList)
    }
    def listToSet: List[Int] => Future[Set[String]] = { inputs =>
      Future.successful(inputs.map(_.toString).toSet)
    }
    def setToIterable: Set[Int] => Future[Iterable[String]] = { inputs =>
      Future.successful(inputs.map(_.toString))
    }
    def listToIterable: List[Int] => Future[Iterable[String]] = { inputs =>
      Future.successful(inputs.map(_.toString))
    }
    def iterableToList: Iterable[Int] => Future[List[String]] = { inputs =>
      Future.successful(inputs.map(_.toString).toList)
    }
    def iterableToSet: Iterable[Int] => Future[List[String]] = { inputs =>
      Future.successful(inputs.map(_.toString).toList)
    }
  ...
  "creates a clump source from various input/ouput type fetch functions" in {
    ...
    def setToMap: Set[Int] => Future[Map[Int, String]] = { inputs =>
      Future.successful(inputs.map(input => (input, input.toString)).toMap)
    }
    def listToMap: List[Int] => Future[Map[Int, String]] = { inputs =>
      Future.successful(inputs.map(input => (input, input.toString)).toMap)
    }
    def iterableToMap: Iterable[Int] => Future[Map[Int, String]] = { inputs =>
      Future.successful(inputs.map(input => (input, input.toString)).toMap)
    }
